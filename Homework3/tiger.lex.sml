...
STRING | COMMENT | SLASH | SLASH_M | INITIAL
...
fun yyAction44 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (let val SOME tempint = Int.fromString(yytext)
                        in
                          Tokens.INT(tempint,
...
fun yyAction47 (strm, lastMatch : yymatch) = (yystrm := strm;
      (nested_comment := !nested_comment + 1;
                        continue()))
...
fun yyQ21 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction63(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"@"
              then yyQ24(strm', yyMATCH(strm, yyAction63, yyNO_MATCH))
            else if inp < #"@"
...
in
  (case (!(yyss))
   of STRING => yyQ0(!(yystrm), yyNO_MATCH)
    ...
    | SLASH => yyQ2(!(yystrm), yyNO_MATCH)
    | SLASH_M => yyQ3(!(yystrm), yyNO_MATCH)
